...
if ($ids_str !== '') {
    ...
    if ($search !== '') {
        $sql .= " AND (s.item_no LIKE ? OR s.username LIKE ? OR s.id LIKE ?)";
        $lk = "%$search%";
...
if ($result->num_rows > 0) {
    // We need to run the update on the same set of IDs
    // Since we already have the result set, let's collect IDs or re-run the criteria
    $update_ids = [];
    $data_rows = [];
    ...
    // Reset result pointer or use the collected rows
} else {
    $data_rows = [];
}
