...
<body class="text-white font-outfit min-h-screen flex overflow-hidden" style="background-color: var(--bg-primary);">
    ...
    <?php $theme_btn_class = (!empty($is_admin) || !empty($is_multi_store_admin)) ? 'flex' : 'hidden md:flex'; ?>
    <button id="theme-toggle-btn" onclick="toggleThemePanel()" 
        class="fixed bottom-6 right-6 z-[90] w-12 h-12 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 text-white <?= $theme_btn_class ?> items-center justify-center shadow-lg shadow-purple-500/30 hover:scale-110 hover:shadow-xl hover:shadow-purple-500/40 active:scale-95 transition-all duration-300 group"
    ...
    <div class="fixed bottom-0 left-0 right-0 z-[100] bg-slate-900/95 backdrop-blur-xl border-t border-white/10 px-1 pb-safe md:hidden shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        <div class="flex items-center justify-between h-16 w-full">
            ...
            <div class="flex flex-1 justify-around h-full">
                <!-- Transact -->
                <a href="history" class="flex flex-col items-center justify-center h-full w-14 transition-colors <?= (isset($page_id) && $page_id === 'history') ? 'text-purple-400' : 'text-gray-400 hover:text-purple-400' ?>">
                    <i class="fas fa-history text-lg mb-1 <?= (isset($page_id) && $page_id === 'history') ? '' : 'text-purple-400' ?>"></i>
                    <span class="text-[8px] font-bold uppercase tracking-tighter text-center leading-tight">Transact</span>
                </a>
                
                <!-- Return -->
                <a href="create_return" class="flex flex-col items-center justify-center h-full w-14 transition-colors <?= (isset($page_id) && $page_id === 'create_return') ? 'text-rose-400' : 'text-gray-400 hover:text-rose-400' ?>">
                    <i class="fas fa-undo text-lg mb-1 <?= (isset($page_id) && $page_id === 'create_return') ? '' : 'text-rose-400' ?>"></i>
                    <span class="text-[8px] font-bold uppercase tracking-tighter text-center leading-tight">Return</span>
                </a>
            </div>
            ...
            <div class="flex flex-1 justify-around h-full">
                <!-- Receiving -->
                <a href="create_receiving" class="flex flex-col items-center justify-center h-full w-14 transition-colors <?= (isset($page_id) && $page_id === 'create_receiving') ? 'text-blue-400' : 'text-gray-400 hover:text-blue-400' ?>">
                    <i class="fas fa-box-open text-lg mb-1 <?= (isset($page_id) && $page_id === 'create_receiving') ? '' : 'text-blue-400' ?>"></i>
                    <span class="text-[8px] font-bold uppercase tracking-tighter text-center leading-tight">Receive</span>
                </a>

                <!-- Theme / Logout Grouping (Split half-half horizontally or use one? Let's use Theme and Logout side-by-side) -->
                <!-- Theme -->
